...
function onBeforeReady(){	 
	this.setOptions({
		initSource : "actnode_queryActnode.action"
	});	 
};
...
function onRowClick(event,rowindex,rowData) {
	c_actnode_id = rowData.c_actnode_id;
	var oper = event.target.name;
	switch (oper) {
	case "add":
		addStep(c_actnode_id);
		return;
	default:
		return;
	}
...
function addStandard(){
	if(!currentActivityOP){
	    ...
	    var ifleaf = activityTree.isLeaf(currentId);
	    if(!ifleaf) {
	    	fastDev.alert('请选择流程子节点！','信息提示','warning');
	    	return false;
	    }
	    else {
	    	doAddStandard("add",currentId,"");
...
function onBeforeDetailGridReady(){
	this.setOptions({
		initSource : "actnode_queryActnodeItem.action?actnode.c_actnode_id="+c_actnode_id
	});
}
...
function doAddStandard(edit,c_action_id,c_actitem_id){
	var src = "addactnode.html?edit="+edit+"&c_action_id="+c_action_id+"&c_actitem_id="+c_actitem_id;
	createWindow({
		src : src,
		title : "标准项信息",
		width : "800",
...
function addStep(c_actnode_id){
	/*var data = fastDev.getInstance("grid1").getValue();
	console.log(data[0].c_actitem_id);
	console.log(data[0].c_actstep_index);
	if (data != null && data.length > 0)
		doAddStep("",data[0].c_actitem_id,data[0].c_actstep_index);*/
	doAddStep("",c_actnode_id,"");
}

function doAddStep(event,c_actnode_id,stepid){
	var src = "stepactnode.html?c_actnode_id="+c_actnode_id;
	var title = "新增步骤";
	if (event=="update"){
...
var reloadActivity = function(){
	...
	if(!currentActivityOP){
	    ...
	    if(!ifleaf) 
	    	...
	    	var grid1 = fastDev.getInstance("grid1");
	    	grid1.addInitReqParam({"actnode.c_actnode_id": currentId});
	    	grid1.refreshData(true);
	    }
