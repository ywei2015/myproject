...
//下拉框
$.get(cqt_prefix+"MaterialUnaccept/feedWorkorderType",function(data,status){
	for(var i = 0;i < data.length;i++){
		$("#workorderType").append("<option>"+data[i]+"</option>");
	}
});
...
function initTable() {
	...
	if(workorderType == "不选")workorderType = "";
	var reg = /-/g;
...
//	console.info(matCode+startDate+endDate);
    ...
    $("#cusTable").bootstrapTable({  
        ...
        url: cqt_prefix+"MaterialUnaccept/feedingStatistic", //获取数据的Servlet地址  
        striped: true,  //表格显示条纹  
