...
function btngetValue() {
  var tree=fastDev.getInstance("taskMouldPojo.c_exec_userid");
	//fastDev.tips("下拉树的选中值是："+tree.getValue()); 
	fastDev.tips("选中的是："+tree.getText()); 
}

function onBtnTest()
{
	var tempetid=fastDev.getInstance("taskMouldPojo.c_tasktemplet_id");
	fastDev.tips("模板ID是："+tempetid.getValue()); 
}
