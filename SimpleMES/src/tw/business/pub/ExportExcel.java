...
public class ExportExcel {
     ...
     * 导出数据
     * */
    public HSSFWorkbook  export(HSSFWorkbook wb) throws Exception{
    
                         // 创建工作簿对象
            HSSFSheet sheet = wb.createSheet(title);                     // 创建工作表
        	...
        	sheet.addMergedRegion(new Region(0,(short)0,0,(short)(rowName.length-1)));//本示例为合并4行2列 
          /*  sheet.addMergedRegion(new CellRangeAddress(0, 1, 0, (rowName.length-1)));  */
            ...
            for(int i=0;i<dataList.size();i++){
                ...
                for(int j=0; j<obj.length; j++){
                   ...
                   /* if(j == 0){
                        cell = row.createCell((short) j,HSSFCell.CELL_TYPE_NUMERIC);
                        ...
                        if(!"".equals(obj[j]) && obj[j] != null){
                        	//时间转换
                        	if(j==(obj.length-1)){
                        		cell.setCellValue(DateBean.timeStrToDate(obj[j].toString()));
                        	}else{
            ...
            for (int colNum = 0; colNum < columnNum; colNum++) {
                ...
                if(colNum == 0){
                    sheet.setColumnWidth((short)colNum,(short)( (columnWidth-2) * 256));
                }else{
                    sheet.setColumnWidth((short)colNum, (short) ((columnWidth+4) * 256));
                }
            }
     ...
     * 导出数据
     * */
    public HSSFWorkbook  exportDatas(HSSFWorkbook wb) throws Exception{
            ...
            HSSFCellStyle columnTopStyle = this.getColumnTopStyle(wb);//获取列头样式对象
            HSSFCellStyle style = this.getStyle(wb);                    //单元格样式对象
        	sheet.addMergedRegion(new Region(0,(short)0,0,(short)(5)));//本示例为合并4行2列 
          /*  sheet.addMergedRegion(new CellRangeAddress(0, 1, 0, (rowName.length-1)));  */
            cellTiltle.setCellStyle(columnTopStyle);
            cellTiltle.setCellValue(title);
            ...
            for(int i=0;i<datas.size();i++){
                ...
                List<Object> obj = datas.get(i);//遍历每个对象
                HSSFRow row = sheet.createRow(i+3);//创建所需的行数
            ...
            for (int colNum = 0; colNum < columnNum; colNum++) {
                ...
                for (int rowNum = 0; rowNum < sheet.getLastRowNum(); rowNum++) {
                    ...
                    if (currentRow.getCell((short) colNum) != null) {
                        HSSFCell currentCell = currentRow.getCell((short) colNum);
                        if (currentCell.getCellType() == HSSFCell.CELL_TYPE_STRING&&currentCell.toString()!=null) {
