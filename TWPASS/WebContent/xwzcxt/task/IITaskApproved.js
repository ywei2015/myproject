...
function onBeforeReady(){
	this.setOptions({
		initSource : "iitask_getTaskInfoListByMapAndOrgid.action?c_status=0"
	});	 
};
...
function doTaskDetails(data){
	fastDev.create("Dialog", {
		...
		showMaxBtn:false,
	    title:"任务详情",
	    src : "IITaskDetail.html?edit=detail&taskId="+ data.c_task_id,
		buttons : [{
			text : "关闭",
			align : "center",
			iconCls : "icon-close",
...
function doModify(data){
	fastDev.create("Dialog", {
		...
		showMaxBtn:false,
	    title:"任务详情",
	    src : "IITaskDetailByApprove.html?edit=confirm&taskId="+ data.c_task_id,
		buttons : [{
			text : "确认下发",
			align : "center",
			iconCls : "icon-save",
