...
import spc.beans.dao.iml.cacheQueryCount.CountCache;
import spc.beans.dao.iml.cacheQueryCount.SqlParams;
import spc.beans.exception.BusinessException;
...
public class GenericHibernateDaoIml implements GenericDao {
	...
	public List getListTest(String queryName, Object[] objs) {
		Session session =sessionFactory.openSession();
		List list = null;
		try {
			final String sql = filterContion(queryName, objs,session);
			final Object[] objects = filterNull(objs);
	...
	public List getNativeListWithOpenSession(final String nativeSql) {
		Session session=sessionFactory.openSession();
		Query queryObject = session.createSQLQuery(nativeSql);
		List list=queryObject.list();
		session.close();
		return list;
	...
	private String filterContion(final String queryName, final Object[] objs,Session session) {
...
//	}
	...
	@Override
	public Object getByIdWithSession(Class clazz, String id) {
		Session session=sessionFactory.openSession();
	...
	@Override
	public Object getObjectBySql(String queryName, Object[] objs) {  
		final String sql = filterContion(queryName, objs); 
		final Object[] objects = filterNull(objs);
		Query queryObject = sessionFactory.getCurrentSession().createQuery(sql); 
		if (objs != null) {
	...
	@Override
	public Object getObjectByNativeSql(String nativesql){     
		Query queryObject = sessionFactory.getCurrentSession().createSQLQuery(nativesql); 
		return queryObject.uniqueResult();
		/*try{
			Query queryObject = sessionFactory.openSession().createSQLQuery(nativesql); 
