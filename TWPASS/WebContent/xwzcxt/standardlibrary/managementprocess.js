...
function addPoint() {
	...
	var selectedNodeId=tree.getCurrentId();
	if(selectedNodeId){
		createWindow({
			src : "filepointadd.html?actionId="+selectedNodeId,
			title : "新增",
			width : "500",
			height : "270",
			buttons : [{
				...
				iconCls : 'icon-reset',
				onclick : function(e,win,cwin, fd) {
					cwin.fastDev.getInstance("addStepForm").resetData();
				}
			}]
		});
	}
...
function openActionDetailsPage(event){
	var src = "actionDetails.html";
	var title = "新增流程节点";
	var eve = "";
	var currentFast=fastDev;
	if(event == "modify"){
		title = "修改流程节点";
		src+="?id="+currentActionId;
		eve=event;
	}
	...
	createWindow({
		...
		buttons : [{
			...
			onclick : function(event, win, childWin, fast) {
				var c_action_fname_input=fast.getInstance("sdActionPojo.c_action_fname"); //文本框
				var c_action_code_input=fast.getInstance("sdActionPojo.c_action_code");	
				var c_remark_input=fast.getInstance("sdActionPojo.c_remark");
				var c_action_pid_select=fast.getInstance("sdActionPojo.c_action_pid");	  //下拉树
				var c_section_id_select=fast.getInstance("sdActionPojo.c_section_id");	  //下拉列表
				
				var fieldsJSON={
					//流程节点表字段
					"pojo.c_action_fname": c_action_fname_input.getValue(),
					"pojo.c_action_code": c_action_code_input.getValue(),
					"pojo.c_remark": c_remark_input.getValue(),
					"pojo.c_action_pid": c_action_pid_select.getValue(),
					"pojo.c_section_id": c_section_id_select.getValue()
				};
				if(eve=="modify"){
					var actionTree=currentFast.getInstance("actiontree");
					var c_action_id=actionTree.getCurrentId();
					fieldsJSON["pojo.c_action_id"]=c_action_id;
					if(!fieldsJSON["pojo.c_action_pid"]){
						fieldsJSON["pojo.c_action_pid"]=actionTree.getParentid(c_action_id);
					}
					fast.post("sdactionnode_updateSdActionToDB.action",{
						
						success : function(data){
							var winner=win;
							var childWinner=childWin;
							var actionId=data;
							fast.post("sdactionnode_updateSdActionPositionsToDB.action",{
								success : function(data){
									winner.close();
									fast.tips("修改成功");
									refreshTree();
								...
								data : {
									"data":childWinner.getValue(),
									"pojo.c_action_id":actionId
								}
							});
						},
						data : fieldsJSON
					...
					if(!fieldsJSON["pojo.c_action_pid"]){
						fieldsJSON["pojo.c_action_pid"]="0";
					}
					fast.post("sdactionnode_addSdActionToDB.action",{
						
						success : function(data){
							var winner=win;
							var childWinner=childWin;
							var actionId=data;
							fast.post("sdactionnode_addSdActionPositionsToDB.action",{
								success : function(data){
									winner.close();
									fast.tips("添加成功");
									refreshTree();
