...
var f_date=getQueryString('f_date'); //test:
var f_pallet_no=getQueryString('f_pallet_no');//test:
var data_p={
		'f_date':f_date,
		'f_pallet_no':f_pallet_no
};
...
function initTable(dataj){
	...
	if(f_pallet_no!=null&&f_pallet_no!=""){
	$.ajax({
		type : "post",
		url:cqt_prefix+'palletreturn/getPalletReturn',
		data:dataj,
			...
			if(length>0){
				for(var i=0;i<length;i++){
					...
					if(rowdata!=null||rowdata!=undefined){
						...
						td=document.createElement('td');
						td.id="picihao";
						data_td1=rowdata.slavebatch;//批次号
						picihao[i]=data_td1;
						td.innerHTML=data_td1;
						r.appendChild(td);
						
						td=document.createElement('td');
						data_td1=rowdata.remark1;//数量
						td.innerHTML=data_td1;
						r.appendChild(td);
						
						td=document.createElement('td');
						data_td1=rowdata.remark2;//单位
						td.innerHTML=data_td1;
						r.appendChild(td);
...
function verifySubmit(){
    	...
    	$.ajax({
    		type : "post",
    		url: cqt_prefix+'palletreturn/deletePalletReturn',
