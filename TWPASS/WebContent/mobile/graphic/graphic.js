...
var dataId = [[null, null], [null, null], [null, null]];
...
//	console.info(time);
	...
	var url = 'statistical_statisticalTask.action?userCode='+userCode+'&x='+x+'&y='+y+'&time='+time+'&flag='+flag+'&dataId='+dataId[0][x - 1];
	$.ajax({
		  url:url,
		  dataType:'json',
...
//	console.info(time);
	$.ajax({
		  url:'statistical_queryErrCount.action?userCode='+userCode+'&x='+x+'&time='+time+'&flag='+flag+'&dataId='+dataId[1][x - 1],
		  dataType:'json',
		  success:function(data){ 
//			  console.info(data); 
			  if(data==undefined||data.length==0){ return; } 
...
//			  console.info(time);
			 $.ajax({
				  url:'statistical_getErrTotalCount.action?userCode='+userCode+'&x='+x+'&time='+time+'&flag='+flag+'&dataId='+dataId[1][x - 1], 
				  dataType:'json',
				  success:function(data){ 
//					  console.info(data); 
					  if(data==undefined||data.length==0){ return; } 
...
function setChart3(x, y, time) {
   ...
   $.post(url,
	      {"userCode":userCode,"time_type":time,"task_type":y,"flag":flag,"dataId":dataId[2][x - 1]},
...
$(document).on("pageshow", "#pageTwo", function() {
	$('#contentTeam').trigger('create');
});

$(document).on("pageshow", "#pageThree", function() {
	$('#contentOrg').trigger('create');
});

function setTableTypeTree(type, id) {
	$.ajax({
		type : 'POST',
		url : 'taskSchedular_getDepartmentTree.action?type=' + type,
		dataType : 'json',
		success : function(data) {
			var html =
				'<div id="-1" data-role="collapsible" data-collapsed="false">' +
				'<h1>所有组织</h1>' +
				'<ul id="ul1--1" data-role="listview" data-inset="true"></ul>' +
				'</div>';
			$(id).html(html);
			if (type == 1) {
				for (var i = 0; i < data.length; i++) {
					if ('团队' == data[i].ITYPE) {
						var _html = '<li><a id="' + data[i].DATAID + '" href="#pageOne" onclick="onAClick(\'' + data[i].DATAID + '\',\'' + data[i].DATANAME + '\',\'' + type + '\');">' + data[i].DATANAME + '</a></li>';
						$('#ul' + data[i].PID).append(_html);
					} else if ('部门' == data[i].ITYPE || '企业' == data[i].ITYPE) {
						var _html =
							'<div id="' + data[i].DATAID + '" data-role="collapsible"' + (('企业' == data[i].ITYPE) ? 'data-collapsed="false"' : '') + '>' +
							'<h1>' + data[i].DATANAME + '</h1>' +
							'<ul id="ul' + data[i].DATAID + '" data-role="listview" data-inset="true"></ul>' +
							'</div>';
						$('#ul' + data[i].PID).before(_html);
				...
				for (var i = 0; i < data.length; i++) {
					if ('部门' == data[i].ITYPE) {
						var _html = '<li><a id="' + data[i].DATAID + '" href="#pageOne" onclick="onAClick(\'' + data[i].DATAID + '\',\'' + data[i].DATANAME + '\',\'' + type + '\');">' + data[i].DATANAME + '</a></li>';
						$('#ul' + data[i].PID).append(_html);
					} else if ('企业' == data[i].ITYPE) {
						var _html =
							'<div id="' + data[i].DATAID + '" data-role="collapsible" data-collapsed="false">' +
							'<h1>' + data[i].DATANAME + '</h1>' +
							'<ul id="ul' + data[i].DATAID + '" data-role="listview" data-inset="true"></ul>' +
							'</div>';
						$('#ul' + data[i].PID).before(_html);
...
function onAClick(id, name, type) {
	for (var i = 0; i < 3; i++) {
		$("#topage" + i).attr("class", "ui-link ui-btn ui-icon-arrow-r ui-btn-icon-top");
	}
	$("#topage" + currentPage).attr("class", "ui-link ui-btn ui-icon-arrow-r ui-btn-icon-top ui-btn-active");
	dataId[currentPage - 1][type - 1] = id;
	if (1 == type) {
		$("#selectTeam" + currentPage).html("当前班组：" + name);
	} else if (2 == type) {
		$("#selectOrg" + currentPage).html("当前部门：" + name);
	}
	switch (currentPage) {
		case 1:
			if ((($("#teamFlip11").find("img")[0].src.indexOf("down.gif") > -1) && (1 == type)) || (($("#departmentFlip11").find("img")[0].src.indexOf("down.gif") > -1) && (2 == type))) {
				setChart1(type, 0);
			}
			if ((($("#teamFlip12").find("img")[0].src.indexOf("down.gif") > -1) && (1 == type)) || (($("#departmentFlip12").find("img")[0].src.indexOf("down.gif") > -1) && (2 == type))) {
				setChart1(type, 1);
			}
			break;
		case 2:
			if ((($("#teamFlip2").find("img")[0].src.indexOf("down.gif") > -1) && (1 == type)) || (($("#departmentFlip2").find("img")[0].src.indexOf("down.gif") > -1) && (2 == type))) {
				setChart2(type);
			}
			break;
		case 3:
			if ((($("#teamFlip31").find("img")[0].src.indexOf("down.gif") > -1) && (1 == type)) || (($("#departmentFlip31").find("img")[0].src.indexOf("down.gif") > -1) && (2 == type))) {
				setChart3(type, 0);
			}
			if ((($("#teamFlip32").find("img")[0].src.indexOf("down.gif") > -1) && (1 == type)) || (($("#departmentFlip32").find("img")[0].src.indexOf("down.gif") > -1) && (2 == type))) {
				setChart3(type, 1);
			}
			if ((($("#teamFlip33").find("img")[0].src.indexOf("down.gif") > -1) && (1 == type)) || (($("#departmentFlip33").find("img")[0].src.indexOf("down.gif") > -1) && (2 == type))) {
				setChart3(type, 2);
			}
			break;
		default:
